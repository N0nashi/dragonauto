...
<div className="flex flex-col gap-2 max-h-56 overflow-y-auto mb-3 pr-1">
        {loading && (
          <p className="font-mont text-xs text-charcoal/90 dark:text-cream/90 text-center animate-pulse py-4">
            {lang === "en" ? "Loading…" : "Загрузка…"}
          </p>
        )}
        {!loading && comments.length === 0 && (
          <p className="font-mont text-xs text-charcoal/90 dark:text-cream/90 text-center py-4">{emptyLabel}</p>
        )}
        {comments.map(c => {

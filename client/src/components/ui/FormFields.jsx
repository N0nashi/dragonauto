...
export const inputCls =
  "w-full font-mont text-sm bg-charcoal/5 dark:bg-cream/5 border border-charcoal/10 dark:border-cream/10 rounded-xl px-4 py-3 text-charcoal dark:text-cream placeholder:text-charcoal/30 dark:placeholder:text-cream/30 focus:outline-none focus:border-red-accent/50 transition-colors";
    ...
    <FormField label={label} required={required} error={error}>
      <div ref={ref} className="relative">
        <button type="button" disabled={disabled}
          ...
          } ${selected.length === 0 ? "text-charcoal/30 dark:text-cream/30" : ""}`}
        >
    ...
    <FormField label={label} required={required} error={error}>
      <div ref={ref} className="relative">
        <button type="button" disabled={disabled}
          ...
          className={`${inputCls} text-left flex items-center justify-between gap-2 ${
            error ? "border-red-accent/50" : ""
          } ${!value ? "text-charcoal/30 dark:text-cream/30" : ""}`}
        >
